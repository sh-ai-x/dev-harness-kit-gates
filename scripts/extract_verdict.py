...
# Original path: templates/ci/scripts/extract-verdict.py  (now lives at: scripts/extract_verdict.py)
# SSOT in the gates repo; copy edits BACK to dev-harness-kit as PRs.
#!/usr/bin/env python3
"""
extract_verdict.py — extract the LLM review/security verdict from
...
Usage:
  python3 extract_verdict.py <claude-execution-output.json>
...
# the combined-verdict-gate PARSE_FAILED arm in the reviewer/security
# judge workflows.
...
def extract_from_comments(path: Path) -> str:
    ...
    push). The reviewer/security wrapper builds the comments file with:

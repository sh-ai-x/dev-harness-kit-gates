...
# Original path: templates/ci/.github/workflows/_verdict_from_comment.py  (now lives at: scripts/verdict_from_comment.py)
# SSOT in the gates repo; copy edits BACK to dev-harness-kit as PRs.
#!/usr/bin/env python3
"""verdict_from_comment.py — fallback verdict extractor from PR comments.

Used by the reviewer/security judge workflows in this repo when the
agent's output file (claude-execution-output.json) exists but cannot
be parsed into a verdict (extract_verdict.py returns the PARSE_FAILED
sentinel -- issue #625). Some providers (e.g. minimax) return a
wrapper-format output envelope that the parser cannot read, but the
agent still posts an `mcp__github_file_ops__create_comment` summary
as a PR comment with a `Verdict: <Word>` line. This helper recovers
the verdict from that comment.

Single-source-of-truth note: VERDICT_RE is mirrored from
`scripts/extract_verdict.py:43` -- same regex, same match group.
Duplicated rather than imported because extract_verdict.py runs in a
different action context that does not have import access to this
helper script.
...
(`**Verdict:** Changes Requested`). The strict extract_verdict.py
...
Usage:
    cat comments.json | VERDICT_COMMENT_CUTOFF=2024-01-01T00:00:00Z \\
        python3 verdict_from_comment.py
...
# Mirrored from scripts/extract_verdict.py:43.
